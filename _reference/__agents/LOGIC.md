...
- Pass 3A list-adapter tightening: FOLEOs list remap now injects class hooks (`foleo-list-page-action`, `foleo-list-views`, `foleo-list-utilitybar`, `foleo-list-searchbox`) and list adapter selectors are gated by `body.foleo-client-shell-active.edit-php.post-type-foleo_page .foleo-page--list` to reduce primitive-first override fights and trim `!important` usage.
- Pass 3B Batch 2 list-adapter hooking: FOLEOs list remap additionally tags title/sort/action elements (`foleo-list-title-link`, `foleo-list-row-title`, `foleo-list-sort-indicator`, `foleo-list-action-btn` with `--primary|--secondary|--delete`) so action/title emphasis styles no longer depend on broad WP primitive selectors.
- Option A shell exception: `edit.php?post_type=foleo_page` is treated as client-shell context even for admin users so FOLEOs list receives `foleo-client-shell-active` and `.foleo-admin-theme-scope`; this exception is screen-specific and does not broaden shell behavior on other admin screens.
- FOLEO v1 screen roots now include explicit page aliases for scoped styling: `foleo-page--workspace`, `foleo-page--new`, `foleo-page--detail`, `foleo-page--setup` (current step default on detail), `foleo-page--assets`, and `foleo-page--setup-shell`. Native FOLEOs list receives optional `foleo-page--list` wrapper class via adapter JS. `foleo-v1.css` is page-root gated under `.foleo-admin-theme-scope` and no longer uses WP body screen-class selectors.
- `foleo-assets` now hosts UI-only workflow screens for `foleo_step=upload|report|preview` (Content Upload, Content Report, Preview placeholder). All scoring/report data is mocked in centralized JS snapshot contracts; rescan swaps snapshots and updates timestamp locally without backend writes.
- Global admin color guardrail now includes expanded `--bd-*` tokens in `assets/css/admin-client.css` (feature panels, alert tones, delete/danger, notes, token-box states, scrims/tooltips/spinner), and page modules should consume those keys rather than introducing new raw hex/rgb values for standard UI surfaces.
